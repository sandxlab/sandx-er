"""DBLP-Scholar benchmark for sandx-er.

Record linkage of academic publications across DBLP and Google Scholar.
This is the large-scale variant: Scholar has 64K records vs DBLP's 2.6K,
making it a strong stress test for the blocking stage.

Dataset (DBLP-Scholar):
    - tableA: 2,616 DBLP publication records
    - tableB: 64,263 Google Scholar publication records
    - links:  5,347 ground-truth matching pairs
    ...
    - id values prefixed A_ (DBLP) or B_ (Scholar)
...
Usage:
    ...
    python -m benchmarks.dblp_scholar --data-path /path/to/dblp_scholar.csv --threshold 0.6
"""
...
def load_data(data_path: str) -> tuple[pd.DataFrame, pd.DataFrame, set[tuple[str, str]]]:
    """Load DBLP-Scholar from a combined CSV with ground-truth cluster_id column.

    Expected CSV columns: id, title, authors, venue, year, source, cluster_id
    - id values are prefixed A_ (DBLP) or B_ (Scholar)
    - Records sharing the same cluster_id across sources are true matches
    """
    ...
    scholar = df[df["source"] == "scholar"].set_index("id")

    print(f"  DBLP    (tableA): {len(dblp):,} records")
    print(f"  Scholar (tableB): {len(scholar):,} records")

    # Build ground truth: cross-table (A, B) pairs sharing cluster_id
    ...
    return dblp, scholar, ground_truth
...
class BenchmarkResult:
    ...
    def __str__(self) -> str:
        reduction = 1 - self.n_candidates / max(1, self.n_records * (self.n_records - 1) // 2)
        return (
            f"\n--- DBLP-Scholar Benchmark Results ---\n"
            f"Records   : {self.n_records:,}  "
            f"(DBLP: ~2.6K  Scholar: ~64K)\n"
            f"Candidates: {self.n_candidates:,}  "
            f"(space reduction: {reduction:.4f})\n"
...
def evaluate(
    ...
    feature_cols = [c for c in ["title", "authors", "venue", "year"]
                    if c in dblp.columns and c in scholar.columns]
    a = dblp[feature_cols].copy()
    b = scholar[feature_cols].copy()
    ...
    for cluster in result.clusters:
        ...
        n_candidates += len(a_ids) * len(b_ids)
        for aid in a_ids:
            for bid in b_ids:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="DBLP-Scholar ER benchmark for sandx-er (large-scale: 2.6K x 64K records)"
    )
    parser.add_argument("--data-path", required=True,
                        help="Path to dblp_scholar.csv (combined DBLP+Scholar with cluster_id)")
    ...
    print(f"Loading DBLP-Scholar from {args.data_path} ...")
    try:
        dblp, scholar, ground_truth = load_data(args.data_path)
    ...
    if blocking_hint := ("SNM on 'title' is fast; LSH scales better on noisy Scholar entries"
                         if args.blocking == "lsh" else None):
        print(f"Note: {blocking_hint}")

    bm = evaluate(
        dblp, scholar, ground_truth,
